...
public class WatermarkDetector {
    ...
    private static final int COARSE_SEARCH_STEP = 1; // Búsqueda píxel a píxel para detectar CROP
    private static final double SCALE_STEP = 0.02; // Más precisión para RESCALE
    ...
    public DetectionResult detect(BufferedImage img, long masterKey, String userId, String contentId) {
        ...
        // Probamos escalas comunes primero (Heurística)
        double[] scales = {1.0, 0.75, 0.5, 1.25};
        for (double s : scales) {
            double z = analyzeScale(img, signature, s);
            if (z > bestZ) { bestZ = z; bestS = s; }
            if (bestZ > 15.0) break;
        }

        return new DetectionResult(bestZ >= 4.0, bestZ, bestS);
    }
    ...
    private double analyzeScale(BufferedImage original, double[][] signature, double scale) {
        ...
        for (int y = 0; y < h; y++) {
            for (int x = 0; x < w; x++) {
                double signal = extractSignal(pixels[y * w + x]);
        ...
        return maximizeCorrelation(folded, signature, w, h);
    }

    private double extractSignal(int rgb) {
        int r = (rgb >> 16) & 0xFF;
        int g = (rgb >> 8) & 0xFF;
        int b = rgb & 0xFF;
        double luma = 0.2126 * r + 0.7152 * g + 0.0722 * b;
        return (double) b - luma; // Correlación de crominancia azul
    }

    private double maximizeCorrelation(double[][] folded, double[][] signature, int w, int h) {
        double maxCorr = 0;
        // Búsqueda de fase (arregla el CROP)
        for (int dy = 0; dy < TILE_SIZE; dy += COARSE_SEARCH_STEP) {
            for (int dx = 0; dx < TILE_SIZE; dx += COARSE_SEARCH_STEP) {
                double corr = computeCorrelation(folded, signature, dx, dy);
                maxCorr = Math.max(maxCorr, corr);
            }
        }

        // Convertir correlación a Z-Score estadístico
        double n = (w * h) / (double)(TILE_SIZE * TILE_SIZE);
        return maxCorr * Math.sqrt(n) * 10;
    }

    private double computeCorrelation(double[][] a, double[][] b, int dx, int dy) {
